...
class Client {
  ...
  async login(context) {
    try {
      ...
      let username = local_username || await vscode.window.showInputBox({placeHolder: 'Username'})
      let password = local_password || await vscode.window.showInputBox({placeHolder: 'Password', password: true})
      this.loading();
