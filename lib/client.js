...
class Client {
  ...
  async createService (context){
    try {
      ...
      this.endLoading()
      // ensure service in runing status?
      this.loading('Running service...')
      await this.checkServiceIsRunning({token, serviceName: service})
      this.endLoading();
  ...
  checkServiceIsRunning({token, serviceName}){
    return new Promise((resolve, reject) => {
      const serviceInterval = setInterval(() => {
      fandogh.services({token}).then(services => {
  ...
  async loading(title = 'Please wait...'){
